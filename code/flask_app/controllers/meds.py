...
def show_all_meds_one_patient_one_pharmacy(patient_id,pharmacy_id):
    if 'patient_id' in session and session['patient_id'] == patient_id:
        ...
        return render_template("patient_meds.html", all_meds = all_meds, one_pharmacy = Pharmacy.get_one_pharmacy(pharmacy_id), one_patient = Patient.get_one_patient({"id": patient_id}))
    else:
